...
<%@page import="in.co.sunrays.beans.SubjectBean"%>
				...
				<table class="functionTable">
					<tr>
						<td class="searchCriteria"><input type="text"
							value="${subjectNameSearch }" name="subjectName"
					...
					<c:if test="${list != null && list.size() > 0}">
						<c:forEach items="${list}" var="bean">
							<tr>
								...
								<td><a
									href="<%=ORSView.SUBJECT_CTL%>?id=${bean.getId()}&operation=<%=SubjectListCtl.OP_EDIT%>">Edit</a></td>
							</tr>
						</c:forEach>
					</c:if>
					<%-- <c:if test="${list == null || list.size() == 0}">
				...
				<table class="bottomFunctionTable">
					<tr>
						...
						<td><c:choose>
								...
								<c:otherwise>
									<input type="submit" name="operation"
										value="${SubjectListCtl.OP_NEXT}" class="commonNextButton">
								</c:otherwise>
							</c:choose></td>
					</tr>
				</table>
