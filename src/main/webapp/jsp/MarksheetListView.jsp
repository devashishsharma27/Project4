...
<body>
	...
	<%
		...
		String studentNameSearch = (String) request.getAttribute("studentNameSearch");
		studentNameSearch = (studentNameSearch == null) ? "" : studentNameSearch;

		String rollNoSearch = (String) request.getAttribute("rollNoSearch");
		rollNoSearch = (rollNoSearch == null) ? "" : rollNoSearch;
	%>
				...
				<table class="functionTable">
					<tr>
						<td class="searchCriteria"><input type="text"
							value="${studentNameSearch }" name="name"
							placeholder="Enter Student Name"></td>
						<td class="searchCriteria"><input type="text"
							value="${rollNoSearch }" name="rollNo"
							placeholder="Enter Roll Name"></td>
					...
					<c:if test="${list != null && list.size() > 0}">
						<c:forEach items="${list}" var="bean">
							<tr>
								...
								<td><a
									href="<%=ORSView.MARKSHEET_CTL%>?id=${bean.getId()}&operation=<%=MarksheetCtl.OP_EDIT%>">Edit</a></td>
							</tr>
						</c:forEach>
					</c:if>
					<%-- <c:if test="${list == null || list.size() == 0}">
			...
			<%-- <div class="bottomFunctionDiv">
				<table class="bottomFunctionTable">
					<tr>
						...
						<td><input type="submit" name="operation"
							value="${MarksheetListCtl.OP_NEXT}" class="commonNextButton"></td>
					</tr>
				</table>
			</div> --%>
			<div class="bottomFunctionDiv">
				<table class="bottomFunctionTable">
					<tr>
						...
						<td><c:choose>
								...
								<c:otherwise>
									<input type="submit" name="operation"
										value="${MarksheetListCtl.OP_NEXT}" class="commonNextButton">
								</c:otherwise>
							</c:choose></td>
					</tr>
				</table>
